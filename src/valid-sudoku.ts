...
function verify(row: string[]): boolean {
  for (let i = 0; i < row.length; i++) {
    ...
    if (parseInt(row[i]) < 0 || parseInt(row[i]) > 10 || row.indexOf(row[i]) !== i) {
      throw new Error(`${{isNAN: !isNaN(parseInt(row[i])), moreThanZero: parseInt(row[i]) < 0 , lessThanten:  parseInt(row[i]) > 10 , unique: row.indexOf(row[i]) !== i }}`);
    }
  }
