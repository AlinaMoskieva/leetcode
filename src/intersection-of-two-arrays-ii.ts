...
function findIntersectionsFrom(elementsStatisticsNums1: Map<number, number>, elementsStatisticsNums2: Map<number, number>): number[] {
  let intersections: number[] = [];

  elementsStatisticsNums1.forEach((frequency, number) => {
    if (elementsStatisticsNums2.get(number) !== undefined) {
      intersections.push(...Array(Math.min(frequency, elementsStatisticsNums2.get(number))).fill(number))
    }
  })
...
function intersect(nums1: number[], nums2: number[]): number[] {
  ...
  return elementsStatisticsNums1.size < elementsStatisticsNums2.size ? findIntersectionsFrom(elementsStatisticsNums1, elementsStatisticsNums2) : findIntersectionsFrom(elementsStatisticsNums2, elementsStatisticsNums1);
};
